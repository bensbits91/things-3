import { TmdbData, GoogleBooksData, GiantBombData } from '@/types/External';

export function normalizeTmdbData(data: TmdbData) {
...
export function normalizeGoogleBooksData(data: GoogleBooksData) {
...
export function normalizeGiantBombData(data: GiantBombData) {
   return data.results.map(item => {
      const { id, name, deck, description, image, original_release_date } =
         item;
      return {
         name,
         type: 'Video Game',
